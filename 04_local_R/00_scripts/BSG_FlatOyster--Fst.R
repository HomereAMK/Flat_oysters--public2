...
ggplot(data = Fst, aes(x = Pop1, y = Pop2, fill = Weighted)) +
  ...
  coord_fixed() +
  geom_text(aes(label = Weighted), color = "#000000", size = 7) +
  scale_fill_gradient2(low = "#80b1d3", mid = "#fccde5", high = "#fb8072",
                       midpoint = Fstmiddle, name = Fst.label,
                       breaks = c(0.02,  0.06, 0.10,  0.14, 0.18, 0.22),
                       labels = c("0.02", "0.06", "0.10", "0.14", "0.18", "0.22"),
                       limits = c(0, .22)) +
...
# Saves plot ~
ggsave(Fst_Plot, file = "BSG_Turbot_Ind66--Fst.pdf",
